    ...
    if descrption_exclude and descrption_exclude != [""]:
        ...
        for point in job.get("bulletPoints", []):
            ...
            if any(exclude.lower() in point.lower() for exclude in descrption_exclude):
                logger.debug(f"Job {job['id']} excluded by description exclude (bullet points)")
                return False
    
    job_description = job.get("description", fetch_job_description(job["id"])).lower()
    
    if (descrption_exclude 
        and descrption_exclude != [""]
        and any(
            exclude.lower() in job_description
            for exclude in descrption_exclude
        )    
    ):
        logger.debug(f"Job {job['id']} excluded by description exclude")
        return False
    
    if (descrption_includes
        and descrption_includes != [""]
        and not any(
            include.lower() in job_description
            for include in descrption_includes
        )
    ):
...
def support_quick_apply(job_id):
    ...
    try:
        ...
        quick_apply_elements = soup.find_all(string=lambda text: "quick apply" in text.lower())
